...
import ddt#数据驱动
import re#正则
#header=myHeader.Get_token_session()
#logging.info(header)
#实例 化日志对象
# 获取所有的测试数据
excelfile=dir_config.testdata_dir+"/acit_1.xlsx"
de=DoExcel(excelfile)
de.write_BtFt()
all_case_datas=de.get_caseDatas_all()
print("所有的测试数据读取",all_case_datas)

@ddt.ddt#ddt的用法
class Test1(unittest.TestCase):
    #① setup():每个测试函数运行前运行② teardown():每个测试函数运行完后执行
    ...
    def test1(self,case_data):
        ...
        res = myRequest.myRequest(case_data["url"], case_data["method"], case_data["request_data"],myHeader.Get_token_session())
        #res=myRequest.myRequest("http://39.108.14.39/operations/user/login","post",case_data["request_data"])
